...
| `chartsearchai.llm.timeoutSeconds` | `300` | Maximum seconds to wait for the inference server's FIRST output. It stops applying once that arrives, so it does not bound the token stream after it — see [Streaming search (SSE)](#streaming-search-sse) |
