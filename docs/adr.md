...
Placed in the prompt's record-type section beside the `Drug reference` and `Safety finding` sentences, and **composed from `PatientChartSerializer.INACTIVE_ORDER_LABEL`** rather than carrying a copy of its text. javac inlines the constant, so no behavioural assertion can tell a copy from a reference — `EndedOrderAnswerRuleTest.thePromptsTriggerTokenIsTheSerializersConstantAndNotACopy` reads the source instead, slicing to the initializer's own terminating quote-semicolon. **Its first version was fail-open and a review round demonstrated it**: it ended the slice at the next `\n\tstatic `, which is a member well past the constant, so a hardcoded copy of the mark passed green as long as the constant's NAME appeared anywhere in between — which a comment can do. Two changes, and neither alone closes it. The scan now stops at the initializer's own terminating quote-semicolon, skipping escaped quotes (the prompt contains `\"` sequences), and asserts it did not run on into the class body. And the case now also asserts the mark's text does NOT appear in the initializer as a Java string literal — because the name can sit in a comment INSIDE the initializer while the prompt hardcodes the text beside it, which is the same fail-open one scope in. Both were closed by reproducing the mutation and watching it go red.
...
| wording | A2 | F (fixture) | B1 | B2 | C1 | E1 dose |
| --- | --- | --- | --- | --- | --- | --- |
| **base** — no clause | ❌ 3/3 | ❌ | ✅ | ✅ | ✅ | ✅ |
| **shipped** — the sentence above | **✅ 3/3** | **✅ 6/6** | ✅ 4/4 byte-identical to base | ✅ 4/4 | ✅ | ❌ lost 3/3 |
...
| + `without dropping anything else you would have reported` | ✅ | ❌ | ❌ | – | ⚠️ | ❌ |
...
- **+** The ticket's own cell is fixed, 3/3 and stable: *"Nevirapine was prescribed, but its order is no longer in force [1]."* Its sibling *"what medications is he taking?"* — recorded in #315 as intermittent, 1/3 — no longer names the drug as if it were current.
...
- **−** **`is he currently taking any medications?` on a chart of one ended order still names nothing** — *"No current medications are recorded."*, unchanged from base. True, and the clause's antecedent is never reached because no drug is named. #315's second comment calls this out as possibly "a second, quieter defect (a shape that answers correctly by declining to name anything)" and defers it to that issue's owner; nothing here decides it.
