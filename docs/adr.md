...
- **+** **The property is structural, so it holds for callers that do not exist yet.** `namingRows` is a function of the question and the patient's orders; any future third `validate` pass agrees with the other two without being told to.
...
- **−** **The two PAIR arms are untouched and keep their own residue.** `addQuestionPairInteractions` and `addActiveOrderPairInteractions` still resolve their subjects through `canonicalSubjects`, over that arm's own rows. What changed is what they can disagree WITH: the shared lookup now folds a narrower set than `resolvedSubstanceRows`, so the residue's boundary moved even though the residue did not. Closing it still means handing those arms this lookup and deleting `canonicalSubjects`, which is #174 site 3's and [#189](https://github.com/openmrs/openmrs-module-chartsearchai/issues/189)'s territory.
