...
- `capture_probe_yesno.sh <outdir>` — the 22 gold patients × 8 topics in the short clinician
  ...
  Since #315 it also fires one yes/no MEDICATIONS cell (`probe-current-meds`, 3.7.1 standalone
...
- `probe_gold_yesno.json` — Tier-B expected leads with adjudication rationale (conditions
  ...
  completeness count to the cohorts a capture actually contains: no host holds both, so counting
  against the whole file would print "capture incomplete" on every run of either one.
