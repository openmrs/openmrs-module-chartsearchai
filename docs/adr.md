...
- **−** **On the renewal chart the answer loses the dose it used to carry** — *"He is taking Nevirapine 400 Milligram"* becomes *"He is taking Nevirapine"*, 3/3. Every wording that keeps that dose pays one of the two prices in findings 1 and 2. It is the lesser loss here because the base arm's own grounding verifier already marked that dose claim UNSUPPORTED, and the answer that replaces it is verified — that citation moves unsupported → supported. It is still a loss and it is not hypothetical.
