...
**A compound claim unit is demote-only, like reference prose but for a different reason.** Where a claim unit attaches its citations to different pieces of its own text — more than one citation, with claim text between two of its `[N]` markers — its statement is a conjunction that no one cited record answers for all of. Asked to entail it, a correct judge says no whether the citation is right or wrong, so publishing that badged correct, active medication citations *Unsupported* on the module's most common question — 8 of the 30 chart citations in [#302](https://github.com/openmrs/openmrs-module-chartsearchai/issues/302)'s 12-patient sweep, all 8 in the colon-less multi-citation row. This rule reaches those whose markers are separated by claim text, so it is not all 8: #302's closing sub-shape (`Salicylic acid [1], [2].`) is co-citation and stays graded. These citations therefore skip Tier-2 (and don't consume its cap), and a Tier-1 cosine pass renders `null` rather than `true`. It applies under entailment only — with Tier-2 off there is no refusal to withhold, so demoting would cost a correct citation its verdict for no defect removed. A cosine *fail* is still published — that is the sentence-scope verdict for a compound sentence that `chartsearchai.grounding.clauseScoped` exists to refine; what #302 removes is Tier-2's negative alone. Adjacent markers are **not** a compound claim: `Infections [5], [12], [15]` is several records cited for one statement (a shape the module's own citation normalizer produces from `[5/12/15]`), where the judge's question is well-formed and its answer is still published.

Each **chart**-group reference in the response carries a `grounded` verdict (`true` / `false` / `null` — not checked, or checked and not certifiable, as for a compound claim unit above), which clients should surface by rendering any citation whose verdict is `false` or `null` as unverified. A `reference`-group citation carries no verdict at all — its `grounded` is always `null`; see below. Grounding never rewrites or blocks the answer — it only annotates which citations could be confirmed.
