...
### A compound claim unit cannot be verified per citation, so it is demote-only (August 2026, [#302](https://github.com/openmrs/openmrs-module-chartsearchai/issues/302))
...
**The decision.** A compound claim unit — more than one attributed citation, with claim text standing between two of its `[N]` markers — makes each of its citations demote-only, exactly as reference prose is, but on the shape of the CLAIM rather than the provenance of the RECORD. They never enter Tier-2 nor consume its per-answer cap, and a Tier-1 cosine pass renders `null`. The two rules are independent; a citation can be demote-only under either.

**It applies under entailment only, and that asymmetry with the reference-group rule is deliberate.** The defect is Tier-2's refusal of a conjunction, and with entailment off there is no such refusal: every verdict is then cosine against the claim text, a compound unit's is no different in kind, and sentence scope has always compared against the whole compound sentence — `clauseScoped` is this module's remedy for that and is untouched here. Demoting in that mode would suppress the PASS of a comparison whose FAIL is still published, costing a correct citation its verdict for no defect removed. It is not a small population either: #302's sweep puts 10 of 30 chart citations in the colon-less multi-citation row, of which this rule reaches those whose markers are separated by claim text — not the co-cited `Salicylic acid [1], [2].` pair the issue folds into that 10.
...
- **It does not silence a cosine FAIL.** A compound sentence's citation is flagged by sentence scope today, and `CitationGroundingVerifierTest.clauseScoped_groundsFirstCitationAgainstItsClauseNotTheCompoundSentence` demonstrates that — but it runs Tier-1-only, where the demotion is gated off, so it does not guard the carve-out. `compoundClaim_anOffTopicCitationIsStillFlagged` does, for this arm; the reference-group arm has its own off-topic cases. Only Tier-2's negative is removed — the one that fired regardless of the evidence.
...
**Costs, and what stays open.** These references move off Tier-2 and onto the lazy Tier-1 path, and that is a swap only where the whole answer is compound. Measured by A/B through the real `verify` over a 12-citation answer with entailment on, counting the real embedder and LLM calls: one compound line citing all 12 drops the batch entirely (1 `entailsBatch` call to 0, 12 pairs to 0) and pays 13 embedding forward passes in its place; 8 sentences of which 4 are compound, covering 9 of the 12 citations, keep the batch for the rest (1 call to 1, 12 pairs to 3) and pay the same 13 passes **on top** — purely additive, on the mixed shape that produced #302's own citations. Those passes serialize on querystore's `synchronized` embed. Tier-1-only mode adds none, and `clauseScoped` is unchanged. There is a second degraded deployment besides the embedder-less one: entailment enabled but the judge unavailable, where every other citation falls back to a published cosine verdict while a compound unit's pass is still demoted. The gate keys on the CONFIGURED mode rather than on whether the judge answered, deliberately — with entailment on the module's promise is a judge-backed verdict and a compound claim can never obtain one, whereas with entailment off it promises only cosine. Conditioning instead on "Tier-2 reached at least one verdict for this answer" was considered and rejected: #302's own headline answer is a single compound sentence, which produces no Tier-2 verdicts at all, so that proxy would switch the fix off for exactly the case it was written for. A subject/polarity flip inside a compound claim is no longer flagged; today it is flagged only by a verdict that flags every citation of that shape alike, which is what the sweep above found on correct records. A clause-scoped fragment's cumulative prefix still names earlier items while being attributed to one citation, and that residual is not closed here. Nor is [#284](https://github.com/openmrs/openmrs-module-chartsearchai/issues/284), whose measured case is a one-marker answer plus a citation present only in the structured array — a claim unit citing one record, so untouched.
...
- **−** The first "+" above does not extend to a compound claim unit: there the entailment tier is not asked, because the question it can ask is not the citation's own, so those citations are demote-only and a flip inside one goes uncaught. That is the #302 trade — an honest silence in place of a refusal published on every citation of the shape, right or wrong.
