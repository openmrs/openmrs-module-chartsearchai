...
- **−** **An existing chip can move where the answer matched a row the patient's orders also carry, with no answer-ONLY row involved.** `resolvedSubstanceRows` builds a group from `inPlay` first and appends order rows after, so such a row sits at its answer position in `resolvedRows` and at its `orderEntries` position in `namingRows` — the same objects in a different ORDER. `DrugReference.canonicalRow` keeps the incumbent on a full tie, so for a family tying on both of its rungs the group's order decides. Nothing in the api suite reddened (1601 tests), so this is stated as reachable rather than as observed.
