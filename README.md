...
**`drug` identifies a substance, not a finding, so it is not a deduplication key.** It carries the drug's display label (possibly with a parenthesized generic synonym) — the name *this response* gives that substance, deliberately the same string across every chip about it since [#206](https://github.com/openmrs/openmrs-module-chartsearchai/issues/206), so one substance is never named two ways in one answer. Several warnings about one substance therefore share it: [#238](https://github.com/openmrs/openmrs-module-chartsearchai/issues/238) records a patient carrying seven hydrocortisone chips that a client collapsing on `(type, drug)` would reduce to one, discarding six distinct findings. This paragraph used to invite exactly that, saying `drug` was there "for grouping, sorting, and deduping". A per-substance heading may group on `drug`; per-finding identity has to key on `detail` (the field that varies between warnings about one substance — it names the interacting order, the allergen or the ceiling), or on the whole warning.
