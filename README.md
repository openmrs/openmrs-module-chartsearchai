...
An additive, opt-in research feature that can inject labelled medication-reference context and run a deterministic post-answer check. Every result reports `checked`, `limited`, or `unavailable` plus package identity and coverage. Only a package explicitly marked `clinically_approved` may produce a product warning. The bundled JSON, DDInter sample, cross-reactivity seed, and configured ATC classification are currently `proposed`; they therefore return a visible limited result and cannot produce deterministic warnings. See [Drug-reference injection & safety validation](#drug-reference-injection--safety-validation) and the safety-boundary corrections on [ADR Decision 23](docs/adr.md#decision-23-drug-reference-injection--post-answer-drug-safety-validation) and [Decision 24](docs/adr.md#decision-24-drug-reference-as-a-pluggable-consumer-of-authoritative-datasets).
...
| `chartsearchai.drugReference.sourceFormat` | `json` | Research data adapter. `json` reads the bundled or configured research schema; `atc` consumes an ATC classification export (`<atcCode> <name>` per line); `ddinter` reads the DDInter-shaped interaction dataset. Selecting a format does not approve its content. ATC is classification/candidate-discovery evidence only and cannot activate interaction, contraindication, duplicate-therapy, or cross-reactivity warnings. Any unrecognized value is treated as `json` |
| `chartsearchai.drugReference.crossReactivityGroupsFilePath` | `chartsearchai/cross-reactivity-groups.json` | Relative path to research candidate groups. The bundled file is explicitly unreviewed and cannot activate deterministic warnings. Editing takes effect on the next module restart |
...
| `chartsearchai.drugSafety.validateAnswers` | `true` | Enable the post-answer check. It always reports an honest status when the feature is enabled; warnings are possible only with a `clinically_approved` package. It never rewrites or blocks the answer |
...
| `chartsearchai.drugSafety.warnOnInteractions` | `true` | Flag a drug in play — asked about in the question, or named by the answer on its own authority — that interacts with one of the patient's active orders, by dataset rule, shared ATC subgroup (duplicate therapy), or shared curated cross-reactivity group (additive / duplicate-class therapy across branches); most specific match wins. Two further joins are not that drug-in-play-against-an-active-order shape: several drugs the QUESTION names are checked against each other (a reference lookup, so neither side need be a drug the patient takes — [#114](https://github.com/openmrs/openmrs-module-chartsearchai/issues/114)), and a question that asks to be screened for interactions but names no drug has the patient's own active orders checked against each other (a chart drug on *both* sides, [#113](https://github.com/openmrs/openmrs-module-chartsearchai/issues/113)). Both of those are pairwise, so both report most severe first, capped by `chartsearchai.drugSafety.maxPairChips`, with the withheld pairs logged |
| `chartsearchai.drugSafety.warnOnContraindications` | `true` | Flag a drug that is contraindicated by an active allergy or condition: by dataset rule, by a recorded allergy to **that same drug**, by an allergy sharing its ATC subgroup, or by an allergy in the same curated cross-reactivity group (cross-branch). Most specific match wins. The same-drug case is identity, not classification, so it needs no ATC code and fires for a drug the dataset cannot classify at all ([#135](https://github.com/openmrs/openmrs-module-chartsearchai/issues/135)). Two joins: a drug in play — asked about in the question, or named by the answer on its own authority — and, on every question, each of the patient's **own active orders**, because the in-play framing could not ask "is the patient allergic to something they are *taking*?" and echo scoping actively withheld it ([#143](https://github.com/openmrs/openmrs-module-chartsearchai/issues/143)); an order already in play is checked once, and the order join stands down entirely for a patient with neither an allergy nor a condition record |
| `chartsearchai.drugSafety.minInteractionSeverity` | `minor` | Within a clinically approved package, minimum source-assigned severity (`unknown` < `minor` < `moderate` < `major`) a rated rule-based interaction must carry. It has no effect on proposed research data, which cannot emit warnings |
...
Each reference in the response carries a `grounded` verdict (`true` / `false` / `null` when not checked), which clients should surface by rendering any citation whose verdict is `false` or `null` as unverified. Grounding never rewrites or blocks the answer — it only annotates which citations could be confirmed.

**Drug-reference citations are demote-only.** A citation of an injected `drug_reference` record is never marked `true`: an answer that recites reference prose overlaps its source near-identically even when it swaps subject roles, so neither cosine nor the entailment judge can vouch for faithfulness there ([#106](https://github.com/openmrs/openmrs-module-chartsearchai/issues/106)). These citations skip Tier-2 entirely (and don't consume its per-answer cap); Tier-1 still flags an off-topic one as `false`, and everything else renders `null` (unverified). Faithfulness of reference content is checked deterministically by the [safety validator](#drug-reference-injection--safety-validation)'s warnings instead.
...
When `chartsearchai.drugReference.enabled` is `true` (off by default), three additive stages run around the answer. All current bundled sources are research-only, so the status is limited and no deterministic warning is emitted:

- **Injection (pre-answer)** — matching entries may be appended as numbered `drug_reference` records. A non-approved package renders classification plus an explicit research-only notice; it does not expose dosing, interaction, contraindication, duplicate-therapy, or cross-reactivity rules to the model.
...
- **Safety check (post-answer)** — returns `safetyCheck` (`drug_safety.v1`) with terminal status, package metadata, mapping/exposure/execution coverage, identity confidence, issues, and any warnings. Missing context or source is `unavailable`; incomplete mapping/exposure or a non-approved package is `limited`. The mature rule engine runs only for a `clinically_approved` package. ATC hierarchy or class membership alone cannot activate a warning. `safetyWarnings` remains as a compatibility projection of approved findings.
...
Render the two groups distinctly: a `reference` entry is a pointer into a drug knowledge base, so presenting it alongside chart records without distinction lets module-supplied text read as chart evidence. Prefer `group` over testing `resourceType` against `drug_reference` client-side — that is what `group` is for, and it keeps the classification in one place when another kind of injected record is added. Do not use `group` to hide references — a `reference` entry is the disclosure of where a drug-interaction statement came from, and the answer prose still cites it by number. Note that a **drug-reference** entry is never `grounded: true`, because those citations are [demote-only](#citation-grounding); their faithfulness is checked by the `safetyWarnings` chips instead. That guarantee is keyed on **`group`** — every `reference`-group type is demote-only, which today means `drug_reference` and `safety_finding` (the module's own deterministic safety findings, injected as citable records since [#110](https://github.com/openmrs/openmrs-module-chartsearchai/issues/110)). The withholding happens server-side, so a `reference`-group citation never arrives as `grounded: true` however the client is written; what a client must not do is treat those types differently from each other, because the withheld pass arrives as `null` and a client that expects `true` for a verified safety finding will show it as unverified forever. Demote-only nulls a passing verdict, it does not suppress a failing one, so `grounded: false` still appears on these and still means the citation is not about the record. It was decided against for `active_drug_order`: that record asserts one drug of this patient, with no subject roles to swap, so a passing verdict is real assurance and it is verified normally.
