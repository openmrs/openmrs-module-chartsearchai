...
One prescription therefore became one duplicate-therapy chip **per ATC code**, each named by whatever the reference data could resolve that code to on its own — the substance where the code is covered, an unlabelled code where it is not. On the shipped knowledge base the codes are usually covered, so the common symptom is one prescription presented as several different medications rather than an unreadable label; the bare code needs a code the dataset does not carry. Measured through the real `validate` with the curated seed: a nameless order carrying `M01AE02` and `M01AE04`, asked about ibuprofen, raised **2 chips**; the same order with a display name raised **1**.
