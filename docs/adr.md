...
- **+** Reachability is curated-source-only by construction: neither `DdiDrugReferenceSource` nor `AtcDrugReferenceSource` publishes a contraindication field at all (0 rules over the DDInter sample's 16 parsed entries; the shipped seed publishes 10 rules, 3 of them self-named allergy rules). So no bundled `ddinter` or `atc` load renders a third section, and the lazy resolution means such a load never resolves the set either.
