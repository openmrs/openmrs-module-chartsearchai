...
| `chartsearchai.drugSafety.maxPairChips` | `10` | Most chips one question may raise from a *pairwise* check (the question's own drugs against each other, or — for a screening question naming no drug — the patient's active orders against each other). Both grow as N²/2, and every chip is also injected as a citable finding, so a cap is required: unbounded, one 16-drug question produced 72 chips and 42,708 characters of finding text. But the number is a clinical judgement, so it is yours: a polypharmacy review clinic may want 30, a triage screen 5. The cost of a low cap is real — measured on the full DDInter data, a 16-drug question shows 10 of 72 pairs and withholds `[Major ×13, Moderate ×40, Minor ×9]`, i.e. thirteen withheld Majors. What is dropped is always the least severe; every withheld pair is named in a WARN, and the counts also reach the response as `interactionPairs` so a clinician can tell a bounded list from a complete one ([#336](https://github.com/openmrs/openmrs-module-chartsearchai/issues/336)). Unparseable or non-positive values fall back to the default ([#131](https://github.com/openmrs/openmrs-module-chartsearchai/issues/131)) |
...
`interactionPairs` says how bounded the **pairwise** interaction check behind those chips was — `{ "found": 18, "reported": 10 }`, the candidate pairs it enumerated and the number `chartsearchai.drugSafety.maxPairChips` let it report ([#336](https://github.com/openmrs/openmrs-module-chartsearchai/issues/336)). A capped screen was otherwise indistinguishable from a complete one: the withheld pairs existed in a server-side WARN and nowhere a client could read, so ten chips read as an exhaustive list. Render it as "10 of 18 shown"; a bounded safety list has to say it is bounded, because silent truncation reads as "nothing else was found".

Two readings a client must keep apart. `found == reported` is a positive statement that the screen was **complete**, and `found == 0` is a real measurement (a screen that ran and related no pairs) — while the key being `null` means the producer stated no measurement at all: no pairwise arm ran, the feature is off, or validation failed. Never read `null` as completeness. And `reported` is a count of **pairs, not of chips**: `interaction` chips also come from the drug-in-play check, so a response can carry more of them than this states pairs. The key is always present, on the `/search` response and on the `done` and `grounded` SSE events alike.
