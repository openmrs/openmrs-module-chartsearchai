...
A deployment that sets `chartsearchai.llm.systemPrompt` REPLACES the built-in prompt wholesale and so drops this rule with everything else in it — such a site keeps #315's behaviour. The property's description says so, but **that description cannot reach the sites that have the problem**: OpenMRS writes a module's `config.xml` description onto an existing `global_property` row only when the stored description is NULL, and having SET the property is exactly what makes the row exist, so an upgrade never rewrites it. The module therefore logs a `WARN` at startup whenever the property is non-empty, naming the rule that is being dropped.
