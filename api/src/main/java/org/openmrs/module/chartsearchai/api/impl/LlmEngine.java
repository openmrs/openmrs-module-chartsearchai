...
public interface LlmEngine {
	 ...
	 *
	 * <p>Both implementations spend this on a JDK {@code HttpRequest.timeout()}, which stops applying
	 * once the inference server's response headers arrive; the token stream is then read lazily out of
	 ...
	 * calls both implementations make here, {@code HttpRequest.timeout(...)} and
	 ...
	 * then stalls, on Java 11, 17 and 21 alike and with {@code ofString()} as well.</p>
	 *
	 * <p>The blocking forms above keep the "maximum wall-clock seconds" wording deliberately: whether
	 * a server that does not stream withholds its headers until the answer is complete was not
	 * measured, and if it does then that wording holds there.</p>
	 *
	 * @param systemPrompt the system prompt
	 * @param userMessage the user message (patient records + question)
