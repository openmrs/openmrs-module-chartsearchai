...
A deployment that sets `chartsearchai.llm.systemPrompt` REPLACES the built-in prompt wholesale and so drops this rule with everything else in it — such a site keeps #315's behaviour with no warning anywhere, which is why the property's own description now says to re-derive a custom prompt from the built-in one after upgrading.
