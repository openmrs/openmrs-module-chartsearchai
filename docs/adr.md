...
**Status: Accepted** (August 2026) — implemented, issue [#317](https://github.com/openmrs/openmrs-module-chartsearchai/issues/317). Adds one trailing field to every `drug_order` chart line and one structural flag beside it; the numbering skips 45, which is left to the open PR #316 so that a gap is paid rather than a collision.
