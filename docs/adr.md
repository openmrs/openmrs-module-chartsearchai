...
- **+** Both fields are assigned in exactly one place — `grep 'namingOrder = '` is the whole audit — so the pair cannot be written inconsistently by any of its write sites.
