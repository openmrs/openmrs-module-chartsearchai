...
- **A behavioural test for a field REASSIGNED once per pass.** There is none to write: such a field sweeps exactly as often as a local does, which is the limit `CLAUDE.md` already records for the analogous `recordedAllergens` memo. Scope that claim to the reassigned shape — the OTHER field shape, initialised lazily and never reassigned, is already caught behaviourally, and by cases that have nothing to do with this change: holding the memo in a `private CoMedications` assigned only when null reddens 4 of the 1555 api tests, among them `FoldedChipOnePartnerNameTest.aNamelessOrderCarryingTwoSubstancesCodesNamesTheClassSentenceAfterTheRulesDrug`, because the second patient of a run reads the first one's chart. The reassigned shape is what the source scan is for, and it asserts the class's whole FIELD BUDGET — one mutable field, the injected service — rather than looking for the memo's own type: three successive reviewers defeated it in turn and each fix opened the next, all three green across the whole suite: the type-shaped version by moving the memo's STATE to an outer `List<OrderPartner>` field while leaving the construction and both call sites alone; the field-budget version by giving that field a parenthesised INITIALISER, which met an exclusion meant for method declarations; and that fix by prefixing the declaration with `@SuppressWarnings("…")`, whose own paren the match-start could not cross. The needle now states what it MATCHES rather than what it excludes — a class member, any leading annotations, a run to a declarator that may not cross a `(` — and the assertion reads the matched text rather than counting. **Treat no list of evasions as closed**; a shape that gets through is a new needle, not a looser one.
