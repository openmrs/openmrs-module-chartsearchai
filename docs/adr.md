...
All three moves are **lateral** in route terms: the first two elect a row naming no route where the incumbent did too, and the third elects one that does not where the incumbent did not either — *neither* tick-borne row is unqualified, so that rename is not a move "toward the unqualified row" and rung two does not only ever hand off to a `namesNoRoute()` row. The baseline side was measured by driving this same method as it stands on `main`, from a second worktree, rather than by re-expressing it.
