...
**One rule is not one chip: a collapsed KEY is a MAX over its rules.** This is the part that was got wrong first, and it was found by a reviewer CONSTRUCTING the pair rather than reading for it. `contraindicationFinding` keys two self-named allergy rules of one entry on the SUBSTANCE ([#146](https://github.com/openmrs/openmrs-module-chartsearchai/issues/146)), so they are one chip and one rendered clause, while each rule is put to the chart on its own token — and they can disagree. The injected record already resolved that as a MAX (*"one corroborated rule of the key is enough for the key"*); the first cut of this change let the answer ride on the rule that won the ledger by RANK instead, which is not the same resolution. It fails on a tie the rank cannot see: `contraindicationRank` answers `SELF_NAMED_RULE_WITHOUT_A_NOTE` for a blank note **without asking corroboration at all**, both disqualifications share the value 0, and the incumbent-keeps tiebreak then leaves the uncorroborated rule's sentence standing. Driven through the real `injectRecords`, one injection carried
...
— two citable records of one chart in flat contradiction, i.e. exactly the failure this decision exists to close, re-created one level down. `ContraindicationChips.add` now folds the answer and stamps it onto whichever sentence the rank chose, reading it off the CHIP so no call site can supply it wrongly; `SafetyWarning.withUncorroboratedChartMatch` is the copy that makes an immutable warning stampable. `UncorroboratedFindingProvenanceTest.oneCorroboratedRuleOfACollapsedKeyClearsTheClauseForTheWholeKey` pins it over a fixture authored for the tie; mutate the AND away and read the failure.

**And the fold is scoped to ONE ENTRY, which is the correction the first version of it needed.** The ledger's key is the SUBSTANCE and therefore spans every ROW of it, while the record is rendered per entry and takes its MAX over that entry's rules alone. Folding across the wider unit re-created the same contradiction with the roles reversed: on two rule-bearing rows of one substance — the shape `DrugReferenceValidity.RULES_WITHOUT_A_SUBSTANCE_IDENTITY` actively instructs operators to author, and which the `json` source reaches because Jackson binds `substanceName` straight onto `DrugReference` — a corroborated rule on the gel row cleared the flag while the tablets row's own record went on hedging the tablets rule's clause, in one injection. So `RaisedChip` carries the ORIGIN entry beside the answer: chips from one origin fold together, and a sentence from another entry that wins the rank brings its own answer with it. `aCorroboratedRuleOnANEIGHBOURRowDoesNotClearTheClauseThatRowsOwnRecordStates` pins that; mutate the origin comparison to always fold and read the failure.

It is worth naming how both halves were found, because neither came from the suite: each was **constructed** by a reviewer who set out to build the contradicting pair rather than to read for it, on a change whose whole subject is two channels agreeing. That is the same lesson Decision 42 recorded about its own third version, one level down.
