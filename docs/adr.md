...
- **−** **The defect was never measured live and still has not been.** No count over the shipped knowledge base is published here: it needs a family whose rows publish different alias sets, which the module's own data statements say is not the usual shape, and the property being bought — that the two passes agree — does not depend on how often they would otherwise have differed. A frequency would be a nice-to-have; asserting one without measuring it through the real predicates is what `CLAUDE.md` forbids.
