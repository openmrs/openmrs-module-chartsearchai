...
- **It does not silence a cosine FAIL.** A compound sentence's citation is flagged by sentence scope today, `clauseScoped` is the existing remedy for that, and `CitationGroundingVerifierTest.clauseScoped_groundsFirstCitationAgainstItsClauseNotTheCompoundSentence` pins it. Only Tier-2's negative is removed — the one that fired regardless of the evidence.
