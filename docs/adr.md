...
- [Decision 49: The injected record names a folded chip's partner in its own vocabulary](#decision-49-the-injected-record-names-a-folded-chips-partner-in-its-own-vocabulary)
...
- **−→+** The `drug_reference` record's interaction-note list named the partner by `partnerLabel` while the chip took the ladder's name, so where the fold reconciled the chip and that record named one partner two ways — the property `CLAUDE.md` states for `partnerLabel` and which #292 asked a fix to keep. **Closed by Decision 49** ([#297](https://github.com/openmrs/openmrs-module-chartsearchai/issues/297)); everything below is what the deviation was and why it was taken, kept because Decision 49 is the answer to it. It was NOT for want of plumbing: `DrugReferenceInjector` holds the validator bean and the service, and `orderedInteractionNotes` already receives `orderEntries`. The reason was that the note is PROMPT text and this repo has rejected four prompt changes on the eval gate for stronger motives than legibility. An earlier form of this bullet added that `displayLabel()` "forbids itself there anyway", quoting its own javadoc's "never used in prompt text" — **that was false and both are now corrected**: `interactionWarning` builds a chip's drug and detail from `displayLabel()` and `renderFinding` copies both verbatim, measured as `Safety finding — Acetylsalicylic acid (aspirin): Acetylsalicylic acid (aspirin) interacts with active order warfarin — Major. …`. What is pinned is the narrower property, that the `drug_reference` record's own text keeps `getName()` (`DrugSafetyChipLabelTest.displayLabelNeverLeaksIntoTheRenderedRecordText`) — and that property is exactly why Decision 49 has the two surfaces name one SUBSTANCE rather than share one string. What bounded the trade meanwhile: the prompt's NAME UNION for that partner never GREW — the chip already carried both names and the record carried the rule's — and it can shrink, where the subject is multi-row and the partner's rule sits only on a sibling row, so `orderedInteractionNotes` never renders the token at all (`DrugReferenceInjector.collect`'s own measured residue — 80 of 121 multi-row substances, 2627 partners) and the prompt names that partner by the ladder name alone.
...
## Decision 49: The injected record names a folded chip's partner in its own vocabulary
