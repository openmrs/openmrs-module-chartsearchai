...
- **−** **Curated sources only, by construction.** Neither bundled parser publishes a contraindication rule — 0 over the 2283 entries the real `DdiDrugReferenceSource` parses from the shipped 19 MB knowledge base, 0 over the ATC sample — so no `ddinter` or `atc` load can render this clause, and `ddinter` is the shipped default. The reachable population is a deployment-authored curated file, which is also what makes the live measurement below a `sourceFormat=json` arrangement rather than a default one.
