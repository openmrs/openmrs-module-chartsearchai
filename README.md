...
**A class code the answer states must come from a record it cites.** Neither tier can see the model *edit* a code it was handed: cosine barely moves when two characters change inside an alphanumeric token, and entailment is paraphrase-tolerant, which is the wrong tolerance for a code substitution. Live, the chip said `J01MA` (fluoroquinolones) while the answer, citing that finding's record number, said `J01CA` (penicillins) ([#142](https://github.com/openmrs/openmrs-module-chartsearchai/issues/142)). So an exact token comparison runs after every answer, independently of `chartsearchai.grounding.enabled`: an ATC-shaped code the answer states that appears in **none** of the records it cites is logged at `WARN` with the codes those records do state. It never rewrites the answer and nothing about it reaches the wire — it is a maintainer's signal, not a clinician's. See [ADR Decision 35](docs/adr.md#decision-35-a-class-code-in-the-answer-must-come-from-a-record-the-answer-cites).
