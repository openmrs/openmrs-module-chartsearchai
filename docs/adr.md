...
- **− A tie still refuses, so Decision 39's symptom survives on the tokens no substance claims outright** — 32 of the 72 rules above, 20 of them on `penicillin g`. Deliberate: two substances claiming a name equally cannot say which of them it denotes, and admitting on a tie is exactly the `Omeprazole`/`Esomeprazole` displacement in a different spelling.
