...
* **The prefix cue is a GUARD, not labelling.** The clause identifies the record class by its text beginning `"Drug order:"` before looking for either marker, and querystore's `AbstractServiceOrderRecordSerializer` emits the SAME `". Stopped: "` and `". Action: "` behind `"Referral order:"` and `"Test order:"` — verified from the deployed jar's constant pool. Drop the prefix as redundant labelling and the model reports an ended lab test as an ended prescription. `theRecordPrefixTheClauseIdentifiesTheClassBySurvivesToo` pins it against the real serializer. It is a shared constant like the two markers, so the two sides divide the work: rename the constant and the prompt-side case moves with it and stays green, while that contract case reddens against querystore's actual output — which is the whole point of asserting it through the constant rather than a literal.
* **The no-marker counterpart states what the record RECORDS, and carries no answering instruction.** It reads *"A drug-order record carrying neither marker records no end."* and stops there. For one review round it read *"A drug-order record carrying neither marker is CURRENT, and a question about what the patient is taking now is answered from those"*, and **both halves of that were measured harmful**, n=3 byte-identical per arm against the same prompt with the whole clause excised. Asserting **currency**: on a patient whose Simvastatin order had lapsed by its `auto_expire_date` (`date_stopped` NULL, so querystore renders no marker) beside two live orders, *"is he currently taking any medications?"* went from *"Yes, the patient has current drug orders: Bupivacaine [3] and Lidocaine [4]. The patient also has an older drug order for Simvastatin Co 20mg [8]."* to *"Yes — the patient is currently taking Simvastatin Co 20mg [8]."* — the lapsed drug asserted current **and both live prescriptions dropped**, in the same payload whose own safety chips named those two and not it. The **answering** half: on a chart of 8 active orders and no ended ones the same question lost its verdict lead altogether, *"The patient is currently taking the following medications:"* against a baseline *"Yes — the patient is currently taking the following medications: Advil 400mg [10], …"* — a rule saying which records a medications question is answered FROM out-ranks the #107 verdict rule two paragraphs below it and supplies no lead of its own. Both were found by a clean-context reviewer of the PR, not by any gate here; the second is why this file now carries a yes/no medications cell (below).
...
**Measured after, FIVE arrangements, n=3 byte-identical per cell, against the wording that shipped** (the compiled `DEFAULT_SYSTEM_PROMPT` was read out of `LlmProvider.class`'s constant pool and confirmed byte-identical to the GP text every cell below was captured under, so none of this describes a superseded draft):

1. **The ticket's own chart** — one stopped Nevirapine order, nothing active. All four shapes correct: *"No — the patient is not currently taking Nevirapine, which stopped on 2026-08-24 [1]."* / *"He has been prescribed Nevirapine, which was an order that ended on 2026-08-24 [1]."* / *"No medications are currently being taken. Nevirapine was stopped on 2026-08-24 [1]."* / *"Nevirapine was stopped on 2026-08-24 [1]."*, the last unchanged from baseline.
2. **Stopped Nevirapine beside an ACTIVE Acetaminophen** — the ticket's own named unmeasured gap, and the over-fire refutation: *"Yes — the patient is currently taking Acetaminophen [1]."*, *"He is currently taking Acetaminophen [1]."*, and the prescribed shape labels both: *"The patient has been prescribed Acetaminophen [1] and Nevirapine [6], which was stopped on 2026-08-20 [6]."*
3. **The RENEWAL shape** — stopped Nevirapine 200mg and a live REVISE 400mg, one drug: *"Yes, he is currently taking Nevirapine [1]."* and *"He is currently taking Nevirapine, 400 Milligram [1]. The previous order for Nevirapine, 200 Milligram, was stopped on 2026-08-15 [4]."* — the "Yes" kept, the dose kept, the prior therapy named with its date where the baseline said nothing about it.
4. **An AUTO-EXPIRED Simvastatin beside live Bupivacaine and Lidocaine** — added in review, and the arrangement that refuted the previous wording: *"Yes — the patient is currently taking Bupivacaine [3], Lidocaine [4] and Simvastatin [8]."* and *"The patient is currently taking: 1. Bupivacaine [3] 2. Lidocaine [4] 3. Simvastatin Co 20mg [6]."* The live prescriptions are back and the answer is verdict-led. The **residue** is that Simvastatin is still listed: see the auto-expiry trade-off below, where a wording that says the lapse out loud was measured and bought nothing.
5. **Eight active orders, none ended** — added in review, and the cell whose verdict lead the previous wording cost outright: *"Yes, the patient is currently taking the following medications: 1. Advil 400mg [10] … 8. Solu-Medrol 125mg/5ml [17]."* The lead is back and no order is dropped.
...
Both gates were **re-run on the wording that shipped**, not carried over from the draft that preceded review — a prompt change that is phrasing-sensitive at one word cannot inherit another wording's collateral-damage numbers. Every figure below is a line the scorer printed; quote it with its denominator, which is the scorer's own and not always the cell count:
...
Beat-or-match holds on both. The safety probe's one flip is `betty__safety-warfarin`, an ABSTAIN cell where arm A refused (*"No — Warfarin should not be started due to potential interaction with Ketoconazole…"*) and the candidate abstains. **No mechanism is claimed for that and none is asserted.**

Two things about the first row, because an earlier version of this table published `verdict-led 6/6` and `abstention held 11 → 13 of 13`. The denominator of the verdict-led line is **7**, the ANSWER-cell count, not 6 and not 20 — `score_probe_safety.py` prints it as `over the same 7 ANSWER cells: verdict-led A=6 B=6`, and a bare `6/6` reads as a perfect column when one ANSWER cell is in fact abstaining in **both** arms. And this run's score carries an **integrity flag** that must not be dropped when the numbers are: 5 of betty's cells were labelled on chips alone because her context fetch returned HTTP 400 — `GET /order?...v=custom:(display,…)` throws `Cannot invoke Concept.getName because "this.route" is null` on one of her orders, a webservices conversion failure in this standalone that has nothing to do with the clause and hits both arms identically. `score_probe_safety.py` exits 3 for it. The comparison between the arms is still like-for-like; the absolute labels for those 5 cells are not adjudicated.

**The presence probe now carries one yes/no MEDICATIONS cell**, `dc8560c9-…|probe-current-meds`, because it had none and could not have caught what review caught: the eval `medications` topic is a wh-question `compare_arms.py` excludes from verdict scoring **by name**, and the 8 Tier-A presence topics do not ask about drugs at all. Verified by mutation rather than asserted — captured under the pre-review wording the cell scores `directness=0/1 … want=YES got=NONE`, and under both the clause-excised baseline and the shipped wording `1/1`. `capture_probe_yesno.sh` also now WRITES its `CAPTURE_DONE` marker file instead of only echoing the words, which is why the 65-cell row above could be read at all: `compare_arms.py` and `score_directness.py` both look for that file, so every A/B ever run on this script's captures reported a missing marker in **both** arms and exited 3 — an integrity signal that is always on is one nobody reads.
...
- **−** **Auto-expired orders are not reached, and the clause narrows but does not close what that costs.** querystore carries `auto_expire_date` in the document metadata but renders no marker for it into the record text — measured by RUNNING the serializer, in `QuerystoreOrderTextMarkerTest.anAutoExpireDateAloneIsNotVisibleInTheRenderedText`. So an order that lapsed by auto-expiry carries no end marker and nothing downstream of the text can see it. For one review round the counterpart said such a record **is CURRENT**, and on the arrangement above that was measured to assert the lapsed drug current *and drop both live prescriptions from the same answer* — wrong in both directions, on the question this issue is about. The counterpart now says only that the record **records no end**, and the live prescriptions come back (arrangement 4). What remains is an INFERENCE the model may still draw — it lists the lapsed Simvastatin among the drugs the patient is taking — and it is a residue rather than a claim the module makes. **The obvious wording fix was measured and bought nothing**: a counterpart adding *"it may still have lapsed on an end date the record does not carry"* left that cell reading *"Yes, the patient is currently taking medications: Bupivacaine [3], Lidocaine [4], and Simvastatin Co 20mg [8]"*, identical in substance, while teaching the model about a date no record carries. The deeper disagreement is unchanged and is the argument for the structural fix: `PatientClinicalContextBuilder` reads `getActiveOrders`, and OpenMRS's `Order.isActive()` excludes an expired order — so such a drug gets no chip, no interaction or duplicate-therapy screening and no #118 injection, while the answer may still list it. Chip and prose can therefore disagree on that drug in the worst direction, and the reviewer who found this saw exactly that: the arm's payload named Bupivacaine and Lidocaine in its chips and Simvastatin alone in its prose. It is closed by threading the metadata rather than reading rendered prose, which is deliberately out of this issue's scope.
...
- **−** **The efficacy evidence is five hand-built arrangements on one local model.** No committed gold cell contains a drug order at all, so neither regression gate can corroborate the FIX — they bound collateral damage only, and the one drug-shaped cell now in `probe_gold_yesno.json` gates the verdict LEAD on a medications question rather than whether an ended order is reported as ended. The ticket's own caveat stands: these are not per-shape rates over models or over phrasings generally. Two of the five arrangements exist only because a reviewer went looking for chart shapes the first three did not cover, and both of them found a regression — so the honest reading of "five" is that the arrangement set has never yet been exhausted.
