...
- **+** The strength of a clinical call now rests on the rating the deterministic layer assigned, and both halves of the mapping are pinned by tests over the real pipeline (`SafetyFindingSeverityStrengthTest` drives the real injector over real datasets, one case per rating INCLUDING the `moderate` boundary itself; the full api suite is green at 1300 tests).
- **+** Nothing changes for `major`, `moderate` or unrated findings, which is where the refusals that matter live.
- **+** Measured on the drift-metric safety probe (`capture_probe_safety.sh`, 20 cells over 4 patients, both arms captured on this standalone with everything but the module identical): **verdict-led 6/7 → 6/7, abstained-on-an-ANSWER-cell 1 → 0, abstention held 11/13 → 11/13, unlicensed verdicts 0 → 0**, on one class flip. That flip is `agnes__safety-aspirin`, her own active order and no chip, where the baseline abstains — the defect the probe's fourth blind spot is about — and this branch does not. Reading that gate needed a fix of its own: it counted only YES and NO as verdict-led, so the caution lead scored as the #107 hedge and the same A/B read **verdict-led 6 → 5**, a regression against a branch that had not regressed. `score_probe_safety.py` now carries the caution class, and `fixtures/probe-safety/caution-lead/` pins it from a live capture of this build. Counting the lead needs the licence check beside it (`unsupported_caution`) *and* the lead's own second half: matching only "the drug can be given" credited four hedges that fit the 40-character window and that neither `classify` nor `ABSTAINED` catches ("I cannot determine whether warfarin can be given" among them), which is the #107 hedge scored as its own fix, so `CAUTION_LED` also requires the caution to be named in the same sentence, the whole shape the prompt teaches, and rejects a lead window carrying a subordinating hedge, since a hedge can name a caution too (*"It is unclear whether warfarin can be given, so caution is advised"*). Neither costs a fixture, and both under-count rather than over-count — a lead phrased "but monitor INR" stops counting, which is the safe direction for a gate whose other failure is fail-open.
