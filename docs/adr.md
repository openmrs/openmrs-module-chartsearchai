...
*"The injector's and the inference service's business."* No. Of every input the naming decision reads, exactly one varied between the passes — `answer`. The question is the same string at both sites; `orderEntries` is `findForActiveOrders(rawContext)`, which reads the order's ATC codes and `getActiveDrugNames()`, neither of which `withReferenceNames` touches (it writes only `activeDrugReferenceNames`); and `recordedDrugNames(context)` is `getActiveDrugNames()`. So request-invariance is reachable *inside* `validate`, with nothing carried between the passes and no new parameter on the public entry point. (`mappings` varies too — null in the pre-answer pass — but its only readers are the answer-side echo test and `SubjectMatter`'s cited-text corpus, so it reaches none of the three inputs the fold takes.) Invariance is also stronger than transport: a third caller of `validate` gets the property for free, where a carrier would have to be threaded to it.
