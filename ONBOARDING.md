...
**`GET /drugreferencestatus`** → `{enabled, loaded, inert, entryCount, sourceFormat, configuredSourceFormat, configuredDataFilePath, origin, findings, arms}`.
...
- `arms` says which safety arms the loaded dataset can actually serve — `doseCeilings`, `handAuthoredRules`, `atcCodes`, `interactions`, each with a `coverage` of `published` / `absent` / `unloaded` and a count beside it. `inert` below is the whole-dataset verdict; this is the same question per arm, and the two are independent: the shipped DDInter default is not inert and still reports `doseCeilings: absent`, because it publishes interaction rules and class codes but no dosing. `absent` is about what the dataset publishes — the class legs can still raise chips from ATC subgroups and the curated cross-reactivity groups. Read `coverage`, not the count — the count is `0` for both `absent` and `unloaded`, and only `coverage` separates "we looked and there is none" from "nobody looked".
