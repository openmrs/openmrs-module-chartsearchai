...
## Decision 51: A folded chip reconciles a token two substances name when one of them names it outright
...
Two choices inside it, each pinned by `FoldedChipOnePartnerNameTest.aRuleTokenTheLaddersRowOnlyTiesKeepsItsOwnToken`, which reddens if either is reversed:
...
- **Rows of the ladder's own substance are excluded from the contest rather than contesting it.** Otherwise an uncontested token starts refusing wherever a substance is filed as several rows.
...
- **− The ranking imposes a `matchesDrugName` floor the existence form did not, so in one shape it NARROWS.** `nameMatchStrength` gates on `matchesDrugName`, which does not trim the alias it scans for, while `isNamed` trims both operands — so a curated entry whose own name survives only as a PADDED alias passes the first gate and fails the floor, and a chip that reconciled before this decision keeps both names. Measured both ways on a curated fixture through the real `JsonDrugReferenceSource` and the real `validate`: `Acenocoumarol interacts with active order Warfarin … as active order Warfarin` before, `… active order warfarin … as active order Warfarin` after. Reachable only on an operator-edited `sourceFormat=json` file, and absent from everything bundled — 0 of the shipped KB's 2297 `(token, row)` `isNamed` pairs rank below `NAME_IS_ANOTHER_NAME`, and the curated seed carries no padded alias. Fail-closed rather than mis-attributing. Pinned by `aPaddedAliasLosesAReconciliationItUsedToGet` so it is visible rather than latent. **Open question:** trimming aliases where the loader already normalises them (`DrugReferenceValidity.sanitizeAliases`, #150) closes it, and widens `matchesDrugName` for every caller — a decision on its own evidence.
- **+ A CONTESTED token is admissible only at `NAME_IS_THE_DISPLAY_NAME`, which is what bounds this change downstream.** Every rival reaching the comparison passed `isNamed`, so every rival ranks at least `NAME_IS_ANOTHER_NAME`, and only the top rank can strictly outrank that. Measured over the shipped KB: of the 18 contested pairs the predicate admits, 18 are at the top rank and none has a `displayLabel` differing from the token by anything but case — so all 28 reconciliations above are case-only (`ketoconazole`→`Ketoconazole`, `atropine`→`Atropine`, and so on). The prompt's name union for that partner therefore cannot move, and `SubjectRule.partnerKey` case-folds to the label the chip now renders, so #121's key-versus-name gap is not widened by any case this decision adds. The UNCONTESTED path is untouched and still admits at either rank.
- **− 34 of the 62 folded chips still show Decision 39's symptom**, and they are not all ties. 20 (`penicillin g`) and 12 (`antithrombin iii`) are: no substance claims those tokens as its own display name, so nothing says which of them a rule is about, and admitting on a tie would be the `Omeprazole`/`Esomeprazole` displacement in a different spelling. The other 2 are `gabapentin`, where the ranking is decisive and the ladder still loses because `canonicalRow` hands it the weaker claimant — recorded here rather than left to read as a tie, because closing it is a question about `entryForAtcCode`'s pick and not about this predicate.
