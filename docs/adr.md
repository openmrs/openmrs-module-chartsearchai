...
**One write path for the pair.** `OrderPartner.recordNameSource(order, namesADrug)` is the only thing that assigns either field: it sets the flag and admits the order only where the flag is true. Both write sites take it — the four-argument constructor and `nameByOrder`. The invariant *a non-null `namingOrder` means the label IS a name* therefore holds at every statement boundary, and `foldedPartnerLabel` now reads that field FIRST.

Three things this deliberately is not.

- **Not a gate on the constructor's argument**, which is the shape the issue proposed. It does not hold: `namingOrder` is not final and `nameByOrder` already assigns it, so a constructor gate binds constructor callers only and leaves the write `nameByOrder` already is unbound.
...
- **Not extended to `label`.** On the constructor's own path a label that is a bare code is CORRECT precisely when `namesADrug` is false, so folding it into the pair would have to reject the state the ladder's last rung exists to express.
...
- **+** The suite pins it through the real public `validate`, and the failures come in two kinds, which is worth recording because only one of them is this issue's. Mutate `recordNameSource` to admit the order unconditionally: three existing cases redden with a bare `[ATC …]` list in the class sentence beside the rule's token — one prescription named twice, i.e. Decision 39's defect returning, because an order with no names cannot satisfy `namesNamingOrder` and so reconciles nothing. **The failure this decision closes needs that branch to answer yes**, which needs an order that HAS names beside a label that is not one — a blank display on the order rung — and then the bare code goes to BOTH sentences. No existing case reached it, so one was added: `FoldedChipOnePartnerNameTest.aBlankDisplayWithNamesNeverHandsItsCodeToBothSentences`, which under the mutation reads `Ibuprofen interacts with active order A01AD05 — … as active order A01AD05 — …`. It is builder-unreachable and needs the public constructor's latitude, like `aBlankDisplayNeverDisplacesTheDatasetName`, so it is a statement about the invariant rather than about production data.
