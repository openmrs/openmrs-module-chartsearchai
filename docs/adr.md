...
- **−** **Nothing in CI exercises the behaviour.** `EndedOrderAnswerRuleTest`'s two answer cases are opt-in and skip without an endpoint, the convention the three existing LLM suites already use; what runs in CI is that the rule is in the prompt, that it is composed from the constant, and that the oracle rejects the defect text. A regression in what the MODEL does with the rule would be caught by running those cases, not by the build.
