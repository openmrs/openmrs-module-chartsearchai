...
## Decision 45: A prompt cannot tell an ended drug order from a live one
...
So the information the prompt is being asked to reason over **is not in the record text**. This is a property of the evidence, not a wording search that has not yet succeeded — which is why the clause is reverted rather than iterated on again.
...
- **`EndedOrderMarkerContractTest`** — the DATA half of the reverted pair, kept because what it pins is querystore's rendered FORMAT and not the prompt: the raw `". Stopped: "` and `". Action: DISCONTINUE"` markers and the `"Drug order:"` prefix, asserted against the real `DrugOrderRecordSerializer`'s output at a finer resolution than `QuerystoreOrderTextMarkerTest`'s lowercased match takes it. The module keys a safety decision on another module's display prose; a rewording there is worth failing loudly on. The PROMPT half, `EndedOrderAnswerRuleTest`, is deleted with the clause — every case in it asserted a prompt phrase that no longer exists.
