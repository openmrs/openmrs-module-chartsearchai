...
- **A behavioural test for a field REASSIGNED once per pass.** There is none to write: such a field sweeps exactly as often as a local does, which is the limit `CLAUDE.md` already records for the analogous `recordedAllergens` memo. Scope that claim to the reassigned shape — the OTHER field shape, initialised lazily and never reassigned, is already caught behaviourally, and by cases that have nothing to do with this change: holding the memo in a `private CoMedications` assigned only when null reddens 4 of the 1555 api tests, among them `FoldedChipOnePartnerNameTest.aNamelessOrderCarryingTwoSubstancesCodesNamesTheClassSentenceAfterTheRulesDrug`, because the second patient of a run reads the first one's chart. The reassigned shape is what the source scan is for, and it asserts the class's whole FIELD BUDGET — one mutable field, the injected service — rather than looking for the memo's own type: three successive reviewers defeated it in turn and each fix opened the next — the type-shaped version by moving the memo's STATE to an outer `List<OrderPartner>` field while leaving the construction and both call sites alone; then the field-budget version by giving that field a parenthesised initialiser, which slipped an exclusion meant for method declarations. Both were 1556 tests green. The exclusion is gone (a method's name is followed by `(`, so the declarator run cannot reach one anyway) and the residue left is a declaration split across lines, named on the needle. Treat no list of relocations as closed.
