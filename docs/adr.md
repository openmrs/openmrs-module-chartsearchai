...
**The two facts are one value.** `render` takes a `ContraindicationReading` in place of its `boolean patientReading` — and no chart parameter at all, since taking one beside the object re-created the same pair one level up. It carries whether the reading may be stated, the chart it is about, and the allergic-substance key set, which is DERIVED on that object rather than supplied. That is [#298](https://github.com/openmrs/openmrs-module-chartsearchai/issues/298)'s shape, and deriving is what lets it hold without the structural guard #298 needed. **Deriving has to reach the SERVICE too, which the first version missed**: those keys are `substanceGroupKey()` values, which for an entry publishing no substance name is the row itself, so a set resolved from a different `DrugReferenceService` than the rendered entries came from would contain nothing the caller can find and every self-named allergy rule would read as uncorroborated — a service constructor argument left the pair intact one field along. The class is therefore an INNER class reading the injector's own field, and "may state the reading paired with a wrongly-empty set" is then unconstructible rather than merely unlikely. Resolved lazily and memoised there, a per-call object and never a field on the singleton bean ([#172](https://github.com/openmrs/openmrs-module-chartsearchai/issues/172)).
