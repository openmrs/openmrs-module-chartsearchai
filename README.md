...
- **`arms`** is what the loaded dataset can actually check, per safety arm that a dataset is able to withhold ([#285](https://github.com/openmrs/openmrs-module-chartsearchai/issues/285)). `inert` above is the whole-dataset verdict; this is the same question one level down, and the two are independent — the shipped default is **not** inert (2283 entries load) and still reports two arms `absent`, because DDInter publishes interaction rules and class codes but no dosing and no hand-authored allergy/condition rule. `coverage` is `published` (at least one entry carries what the arm needs), `absent` (a dataset was read and **no** entry carries it, so the arm cannot fire) or `unloaded` (nothing was read, so nothing is known). Each arm asks what its own check asks, not whether the field is populated: `doseCeilings` needs an age band publishing a daily or per-kg maximum, not merely an age band; `atcCodes` needs a code that reduces to the level-4 subgroup the class arms compare, not merely a code; `handAuthoredRules` needs a rule typed `allergy` or `condition` with a usable token, since no other rule can be put to a chart. A field that is populated with nothing actionable reports `absent`, which is the point — reporting it `published` would be the same "looks healthy, checks nothing" state one level down. **Read `coverage`, not `entriesPublishing`**: the count is `0` for both `absent` and `unloaded`, and the difference between "we looked and there is none" and "nobody looked" is the whole point. An `absent` arm is not a defect and is not reported in `findings` — a dataset publishing no dosing is valid here, and [ADR Decision 24](docs/adr.md#decision-24-drug-reference-as-a-pluggable-consumer-of-authoritative-datasets)'s matrix records that no free authoritative dataset publishes dosing maxima. It *is* the thing to read before concluding that `chartsearchai.drugSafety.warnOnDoseExcess: true` means doses are being checked. An install that needs them selects `sourceFormat=json` or points `dataFilePath` at a dataset carrying age bands. The mirror case is `sourceFormat=atc`, which sets only class codes, so it reports three arms `absent`. Note what `absent` does and does not say there: it is about what the DATASET publishes, and the class legs still emit interaction and contraindication chips from ATC subgroups and the curated cross-reactivity groups, so `interactions: absent` means "this dataset carries no pairwise rules", not "no interaction warning can be raised". The arms a dataset cannot withhold — a recorded allergy to the drug itself — are deliberately absent from this map, since reporting them would say nothing.
