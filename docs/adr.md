...
1. `PatientClinicalContextBuilder` reads `DrugOrder.getRoute()` and the ordered `Drug`'s `getDosageForm()`, each in its own `try`, and `PatientClinicalContext.ActiveDrugOrder.getAdministrationTerms()` carries them per order. **Both sources**, because neither alone covers the shapes that matter: the route is a `drug_order` column and so is the only leg a non-coded order has, while the dose form is a `drug` column and is the only leg that reaches the skin — measured 2026-08-28, the 3.7.1 reference dictionary's "Route of administration" set has 17 members and not one names a cutaneous route.
   ...
   **Every name the concept publishes, not the one `Concept.getName()` elects.** That method returns the locale-**preferred** name ahead of the fully specified one, and on that same 17-member set three of the nine site-naming routes are preferred under a spelling this module's vocabulary does not contain: concept 874 `In both eyes` (fully specified `Bilateral eye administration`), 877 `In both ears` (`Bilateral ear administration`), and 872 — the only vaginal route there is — `Vaginally` (`Vaginal administration`). `DrugReference.containsWord` takes no trailing letters, so the plural and the adverb miss the singular terms, and the first implementation of this decision therefore left the ticket's own defect standing for the standard bilateral eye and ear routes. Reading them all is the root-cause reading rather than adding those spellings as terms, and it also carries the route-of-**entry** half: the same dictionary prefers `Rectally` and `Per NG tube`, whose fully specified `Rectal administration` and `Nasogastric tube administration` are what `ROUTES_OF_ENTRY` is spelled for. The widening is scoped to a helper this path alone uses — the order-NAME path keeps `addConceptName`, because what counts as a name there decides which reference entries an order can reach at all, which this decision has no evidence about. It is not locale-scoped either: a name in another language contributes nothing to an English vocabulary, while scoping to the session locale would drop the spelling that carries the site wherever a dictionary files it elsewhere. `ActiveOrderAdministrationTermsTest.everySpellingTheRouteConceptPublishesIsCarried` pins it, and it is the one case in that class whose concept has a preferred name differing from its fully specified one.
