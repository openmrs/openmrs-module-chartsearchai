...
- **−** Decision 39's condition list, and `foldedPartnerLabel`'s `<ol>`, now have their FIRST TWO items transposed relative to the code; items 3 and 4 still follow it. Deliberately not renumbered: the ordinals are cited by number in this file and in the javadoc, so they are stable identifiers and the lists say so.
