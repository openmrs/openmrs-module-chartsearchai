...
- **−** **The prompt grew by one clause** (7637 → 8921 chars as compiled), against a default `contextSize` of 32768. Negligible for the window; it is a constant prefix, so llama-server's KV reuse is unaffected. Recorded because the prompt is the one artifact in this module where every addition has to justify itself against `eval/drift-metric`'s record of three reverted arms.
