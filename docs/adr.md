...
- **−** The structural guard's own limits, stated so nobody has to rediscover them: it does not see assignment by reflection, nor a value smuggled in by a form its pattern does not describe; it says nothing about whether the gate is the right way ROUND — `namesADrug ? null : order` still names the flag and does pass here, and is caught behaviourally instead, measured: it reddens `ClassChipPartnerLabelTest.anOrderTheDatasetDoesNotCoverIsNamedByItsOwnDisplayName` and `FoldedChipOnePartnerNameTest`'s renamed-partner refusal, because an order-rung partner then carries no naming order and the fold refuses where it should reconcile; and it is a statement about the source as WRITTEN, so renaming either field means updating its needles — a failure it makes loud, since it asserts it located both declarations, the `OrderPartner` body and `recordNameSource`'s body before asserting anything about the assignments. Its scope is one file, which is the whole scope the compiler leaves open: both fields are private members of a private nested class.
