...
**And the parenthetical it sits in is checked for shape, not only for membership.** Set membership cannot see a code stated four times or a citation marker landing inside the brackets, and both reached a clinician ([#338](https://github.com/openmrs/openmrs-module-chartsearchai/issues/338): `(H02AB, H02AB, H02AB, H02AB)`, `(H02AB [12])`). Neither is decided against the records — nothing this module renders states one code twice inside one parenthetical, and a marker attributes the clause rather than the code — so both are reported at `WARN` from the answer's shape alone, behind the same gates and on the same logger, still without rewriting a word of it. Two different codes in one parenthetical are left alone: the injected drug-reference record renders a substance's own codes exactly that way, from a `Set` that cannot repeat one. See [ADR Decision 59](docs/adr.md#decision-59-a-class-code-parenthetical-is-checked-for-shape-as-well-as-membership).
