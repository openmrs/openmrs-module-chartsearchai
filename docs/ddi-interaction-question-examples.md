...
- **A screening answer can read as a prescribing refusal** ([#348](https://github.com/openmrs/openmrs-module-chartsearchai/issues/348)). [3a](#3a-one-major-pair-on-a-two-drug-chart)
  ...
  by `LlmProvider.DEFAULT_SYSTEM_PROMPT`'s own branches; [ADR Decision 70](adr.md)), so a finding
  ...
  on the two-build A/B [ADR Decision 70](adr.md) records: 3a now answers *"No — Methotrexate should
