...
- The rule must name the entry the ladder resolved, and name it **unambiguously** — one substance, not merely this one among several (`unambiguouslyNames`). Otherwise the two arms may be about different co-medications and **each sentence keeps its own name**, which is what the chip has always said.

Nothing outside a folded chip moves: an unfolded rule chip, a class-only chip, the grouping keys (`SubjectRule.partnerKey`, `DrugReferenceInjector.onePerPartner`) and the injected `drug_reference` note list all keep `partnerLabel` itself.
