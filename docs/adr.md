...
## Decision 40: A partner's name source is one fact, carried by the value rather than by a branch order

**Status: Accepted** (August 2026) — implemented, issue [#298](https://github.com/openmrs/openmrs-module-chartsearchai/issues/298). Behaviour-neutral; it moves a guarantee Decision 39 relies on out of `foldedPartnerLabel`'s statement order and into `OrderPartner`'s state, and retires the statement-order guard in exchange.
...
- **Not `displayNamesADrug` re-asked inside the object.** At the ladder's own call site the predicate and the flag are literally equal, so that form compiles, reads more like CLAUDE.md's "ask it of yourself" rule, and leaves the suite green — and it silently destroys the invariant, because a caller passing `false` beside a real display would keep a non-null order. The fold would then hand that caller's label, which on this rung can be a bare ATC code, to both sentences. Deriving one field from the other is what satisfies the rule's actual demand: the object does not trust a caller to have applied a guard, because there is no second question left to ask.
- **Not extended to `label`.** On the order rung a label that is a bare code is CORRECT precisely when `namesADrug` is false, so folding it into the pair would have to reject the state the ladder's last rung exists to express. What keeps the label and the flag in step instead is the second leg above — each constructor derives both from one source.

**And the branch order is reversed, deliberately.** That is what makes the invariant load-bearing rather than redundant, and therefore what makes it testable at all: measured, the gate alone changes nothing observable through the public pipeline (1347 api tests, 0 failures), so with the branches left as they were the change would ship with no coverage.

### Trade-offs

- **+** The value answers the question. A rung added later may read `namingOrder` in any order without the #155 hazard, which is what the issue asked for.
- **+** Both fields are assigned in exactly one place — `grep 'namingOrder = '` is the whole audit — so the pair cannot be written inconsistently by either existing write site.
- **+** The suite pins it through the real public `validate`, and the failures come in two kinds, which is worth recording because only one of them is this issue's. Mutate `recordNameSource` to admit the order unconditionally and read the failures. Some redden with a bare `[ATC …]` list in the class sentence beside the rule's token — one prescription named twice, i.e. Decision 39's defect returning, because an order with no names cannot satisfy `namesNamingOrder` and so reconciles nothing. No count is given, deliberately: `nameByOrder`'s own comment records that an exhaustive list here went stale twice, in the very round that added another observer. **The failure this decision closes needs that branch to answer yes**, which needs an order that HAS names beside a label that is not one — a blank display on the order rung — and then the bare code goes to BOTH sentences. No existing case reached it, so one was added: `FoldedChipOnePartnerNameTest.aBlankDisplayWithNamesNeverHandsItsCodeToBothSentences`, which under the mutation reads `Ibuprofen interacts with active order A01AD05 — … as active order A01AD05 — …`. It is builder-unreachable and needs the public constructor's latitude, like `aBlankDisplayNeverDisplacesTheDatasetName`, so it is a statement about the invariant rather than about production data.
- **−** **A second, independent guard is given up, and this is a trade rather than a pure win.** With `!namesADrug` tested first, an inconsistent pair could not reach the order branch *whatever produced it*. Now it can, so the invariant is the only thing between such a pair and a code-valued label in both sentences of a chip. The exchange is deliberate: defence in depth resting on a caller's statement order, for an invariant resting on one write path plus test coverage that the statement-order version could not have.
- **−** **A rung that bypasses `recordNameSource` with a direct field assignment is bound by its javadoc and by nothing structural.** `namingOrder` cannot be final while `nameByOrder` renames a partner, so the single write path is a convention the class states rather than one the compiler enforces. This is the residue the bullet above pays for, and it is narrower than what it replaced only in the sense that it is one method away rather than one branch away.
- **−** What the suite pins is the PAIR of changes, not the invariant in isolation. Revert the write path with the branches reversed and the cases above redden; revert both and the suite is green again. Nothing asserts the invariant with the branches in their old order, because nothing can — the change is behaviour-neutral by construction, which is why the issue predicted that no expectation would move, and none did.
- **Considered and declined: deriving `namesADrug`.** Post-change the flag equals `namingOrder != null || labelEntry != null` at every write site, and review measured the derivation green over the whole suite — so it would remove a field and a parameter. Declined on two grounds. It changes what the flag MEANS, from a property of the LABEL (which is what its own javadoc has said since #292) to a function of the label's SOURCE. And its stated benefit is false: with one field there is no pair to disagree, but the property that matters — when the fold hands out `label`, `label` is a name — still rests on only ever storing a naming order, so a rung writing `namingOrder` directly would yield a derived `true` and put a bare ATC code in both sentences, which is the very shape `aBlankDisplayWithNamesNeverHandsItsCodeToBothSentences` pins. The invariant that matters stays conventional either way; the derivation buys a field, not enforcement.
- **−** Decision 39's condition list, and `foldedPartnerLabel`'s `<ol>`, are now in the opposite order from the code. Deliberately not renumbered: the ordinals are cited by number in this file and in the javadoc, so they are stable identifiers and the lists say so.
