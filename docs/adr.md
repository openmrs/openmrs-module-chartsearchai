...
**The two facts are one value.** `render` takes a `ContraindicationReading` in place of its `boolean patientReading`: whether the reading may be stated at all, and the allergic-substance set, which is DERIVED on that object rather than supplied — so "may state the reading" paired with a wrongly-empty set is unconstructible. That is [#298](https://github.com/openmrs/openmrs-module-chartsearchai/issues/298)'s shape, and deriving it is what lets it hold without the structural guard #298 needed. Resolved lazily and memoised there, a per-call object and never a field on the singleton bean ([#172](https://github.com/openmrs/openmrs-module-chartsearchai/issues/172)).
...
- **+** Reachability is curated-source-only by construction: neither `DdiDrugReferenceSource` nor `AtcDrugReferenceSource` publishes a contraindication field at all. Measured through those parsers and `DrugSafetyValidator.selfNamedAllergyRule` rather than read off the schema: 0 contraindication rules over the DDInter sample's 16 parsed entries, against 10 rules on the shipped curated seed's 4 entries, 3 of them self-named allergy rules. So no bundled `ddinter` or `atc` load renders a third section, and the lazy resolution means such a load never resolves the set either.
