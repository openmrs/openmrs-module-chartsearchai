...
**Costs, and what stays open.** These references now take the lazy Tier-1 path that the amortized Tier-2 batch spared them — an embedding pass per record and per claim unit on CPU deployments, the same trade the reference carve-out records for itself. A subject/polarity flip inside a compound claim is no longer flagged; today it is flagged only by a verdict that flags every citation of that shape alike, which is the 8-of-8-on-correct-records finding above. A clause-scoped fragment's cumulative prefix still names earlier items while being attributed to one citation, and that residual is not closed here. Nor is [#284](https://github.com/openmrs/openmrs-module-chartsearchai/issues/284), whose measured case is a one-marker answer plus a citation present only in the structured array — a claim unit citing one record, so untouched.
