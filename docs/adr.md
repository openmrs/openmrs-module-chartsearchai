...
— two citable records of one chart in flat contradiction, i.e. exactly the failure this decision exists to close, re-created one level down. `ContraindicationChips.add` now ANDs the answer across everything raised on the key and stamps it onto whichever sentence the rank chose, reading it off the CHIP so no call site can supply it wrongly; `SafetyWarning.withUncorroboratedChartMatch` is the copy that makes an immutable warning stampable. `UncorroboratedFindingProvenanceTest.oneCorroboratedRuleOfACollapsedKeyClearsTheClauseForTheWholeKey` pins it over a fixture authored for the tie; mutate the AND away and read the failure.
