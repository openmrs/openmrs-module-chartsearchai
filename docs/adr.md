...
- **+** The prompt's name union for a partner still cannot GROW. Every name the note can newly carry is one the same prompt already contained, because the folded chip reaches it verbatim through `renderFinding`. What is lost is the rule's own token where nothing else in that prompt carried it.
- **−** The note's name is longer than the token it replaces (`Acetylsalicylic acid` for `aspirin`, +13 characters), and `render`'s interaction section is a soft budget in which length decides which promoted partner keeps its mechanism prose and how many partners are reported withheld. Segment 1 overrides the budget so a promoted partner is never invisible, so nothing can be DROPPED; what can change is full-versus-compact for a second promoted partner. Not measured at knowledge-base scale, and no existing note-text case can see it — every one of them runs on a validator-less injector, so this change is inert for them by construction.
