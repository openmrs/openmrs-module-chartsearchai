...
- **`origin`** is what was *read* (an absolute path, or `classpath:/chartsearchai/…` for the bundled dataset); `configuredDataFilePath` is what was *asked for*. They differ when the configured file could not be read and the bundled dataset was used — which yields a plausible non-zero `entryCount`, so the count alone does not tell you your file loaded.
